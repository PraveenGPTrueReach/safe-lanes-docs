...
- **Primary User Roles/Access Levels**: Vessel Super Admin, Vessel Admin (with department-limited view), and optionally External (read-only) if configured.  
...
- **Primary User Roles/Access Levels**: Same as V1.0a (Vessel Super Admin, Vessel Admin, optionally External if configured).  
...
- **Integration Points**: Returns to V2.0b for overview;   
- **Primary API Dependencies**:  
...
- **Primary User Roles/Access Levels**: Office User, Office Admin, Office Super Admin (edit if permitted).  
...
- **Note**: no separate office-side task approval workflow is provided. If future organizational policy requires it, an additional approval feature could be introduced without significant rework.  
- **Primary API Dependencies**:  
...
- **Note**: As with O3.0a, there is no office approval mechanism for variable tasks. Organizations can optionally configure overrides if needed.  
- **Primary API Dependencies**:  
...
- Offline usage is required on a mobile device at the vessel
...
| O2.0c | Office (portal) | O2.0b (Edit) | Return to V2.0b | Daily form with office corrections |
| O3.0a | Office (portal) | Main nav "Plan" toggle-Fixed | Back to O1.0a/O2.0a | View-only oversight of vessel tasks (Office can't edit by default) |
| O3.0b | Office (portal) | Main nav "Plan" toggle-variable | Back to O1.0a/O2.0a | View-only oversight of ad-hoc tasks (Office can't edit by default) |
