...
- Vessel Admin can Advance or Retard a day  
  ...
  - If the user clicks on Advance 1 Day checkbox, then the next day is deleted from RH recording sheet (e.g. if date selected is 13th and Advance 1 Day checkbox is clicked, row for recording rest hours on 14th is deleted)  
  - If the user clicks on checkbox next to Retard 1 Day then another row with same date is created in RH recording sheet (e.g. if date selected is 13th and Retard 1 Day checkbox is clicked, an additional row with same date, 13th is created below 13th for recording rest hours)  
