...
This section details how the FE is organized internally (modules, services, and core logic), how data flows through it, and how performance and testing are addressed. Although Angular 18.2.0 is the framework of choice, the FE is kept lightweight to avoid overcomplex solutions.
...
2. Daily Rest-Hour Saving:  
   ...
   - If the back-end flags compliance violations, the FE displays them in real time.  
   - 
...
- Global Layout:  
  ...
  - Sidebar or top nav links lead to Daily Logs, Planning,, and Reports (if allowed by role).
...
Below are minimal endpoint summaries relevant to the FE. All calls require authentication (JWT).
...
The FE obtains a JWT token (both vessel as well as office environments) from the respective SafeLanes Identity Service, as described in higher-level documents. The token is stored in memory (AuthService). The OfficeBE itself does not issue JWT tokens; it merely validates them when performing data operations.
...
- All overwritten data is stored in the back-end’s audit log, with read-only references for Admins.
...
- Data Consistency:  
  ...
  - If offline, day-level edits are stored in the vessel MySQL via local endpoints. 
