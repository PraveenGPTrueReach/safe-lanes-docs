...
4. Audit Logging Module  
   ...
   - Inserts old and new values, plus userId and updatedAt, into an immutable audit\_log table.
...
erDiagram
    ...
    RESTHOURS ||--o{ AUDIT\_LOG : "logged changes"

    TASKS ||--o{ AUDIT\_LOG : "logged changes"
...
For all updates (rest hours or tasks), the new record replaces the old if its updatedAt is more recent. The overwritten version is captured in audit\_log so it remains consultable indefinitely. This approach follows Clarification \#2: No separate conflict table or Admin UI for merging.
...
- Application-level logic inserts an entry in AUDIT\_LOG in the same transaction as the update or delete.  
...
- Environment variables (DB\_HOST, DB\_USER, DB\_PASS, etc.) differ slightly between vessel and office.  
...
- Keep all overwritten data in audit\_log with userIds, timestamps, and oldValue/newValue for traceability.  
...
- Each rest-hour or task record update commits in its own transaction, logging the old version in AUDIT\_LOG if needed.  
